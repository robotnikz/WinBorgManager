    ...
    <div className="bg-white rounded-xl border border-gray-200/75 p-5 shadow-sm hover:shadow-md transition-all duration-200 group">
      ...
      <div className="flex gap-2 mt-2">
         <button 
           ...
           className="flex-1 px-3 py-2 text-xs font-medium bg-gray-50 hover:bg-gray-100 border border-gray-200 rounded-md text-slate-700 transition-colors"
         >
           {repo.status === 'connected' ? 'Check Connection' : 'Connect'}
         </button>
         {repo.status === 'connected' && (
            <button 
