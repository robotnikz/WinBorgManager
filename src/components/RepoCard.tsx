    ...
    <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200/75 dark:border-slate-700 p-5 shadow-sm hover:shadow-md transition-all duration-200 group relative overflow-hidden flex flex-col h-full">
      ...
      <div className="flex items-start gap-4 mb-4">
          ...
          <div className="min-w-0 flex-1 pt-0.5">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 truncate font-mono opacity-80 mt-1" title={repo.url}>{repo.url}</p>
          </div>
          ...
          <div className="shrink-0 flex flex-col items-end gap-1">
            ...
            {repo.isLocked && (
                 <div className="flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold bg-orange-50 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300 border border-orange-200 dark:border-orange-800" title="Repo is Locked">
      ...
      <div className="flex items-center bg-gray-50 dark:bg-slate-900/50 border border-gray-100 dark:border-slate-700/50 rounded-lg p-1.5 gap-1 mb-5">
            ...
            <div className="flex items-center gap-0.5 border-l border-gray-200 dark:border-slate-700 pl-1">
                {onMaintenance && repo.status === 'connected' && (
                    <button onClick={() => onMaintenance(repo)} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:text-indigo-400 dark:hover:bg-indigo-900/20 rounded" title="Maintenance">
                        <Wrench className="w-3.5 h-3.5" />
                    </button>
                )}
                {onExportKey && repo.encryption !== 'none' && (
                    <button onClick={() => onExportKey(repo)} className="p-1.5 text-slate-400 hover:text-yellow-600 hover:bg-yellow-50 dark:hover:text-yellow-400 dark:hover:bg-yellow-900/20 rounded" title="Export Key">
                        <Key className="w-3.5 h-3.5" />
                    </button>
                )}
                {onEdit && (
                    <button onClick={() => onEdit(repo)} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:text-blue-400 dark:hover:bg-blue-900/20 rounded" title="Edit">
                        <Edit2 className="w-3.5 h-3.5" />
                    </button>
                )}
                {onDelete && (
                    <button onClick={() => onDelete(repo)} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:text-red-400 dark:hover:bg-red-900/20 rounded" title="Delete">
